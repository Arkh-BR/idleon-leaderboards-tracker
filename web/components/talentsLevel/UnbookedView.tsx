  ...
  return (
    <section className="rounded-lg border border-zinc-800 bg-zinc-950/40">
      ...
      {open && (
        <div>
          {tabGroups.map((tg) => {
            const tabOpen = isTabOpen(tg.tabIndex);
            return (
              <div key={tg.tab}>
                {/* Collapsible tab header — tells the user which class skill
                    tab the talents below belong to (Rage Basics, Warrior, …)
                    and lets them fold each tab away. */}
                <button
                  type="button"
                  onClick={() => toggleTab(tg.tabIndex)}
                  className="w-full flex items-center gap-2 px-3 py-1.5 bg-zinc-900/60 border-b border-zinc-800/60 text-[11px] font-semibold uppercase tracking-wide text-sky-400/90 hover:bg-white/5"
                  title={tabOpen ? "Collapse tab" : "Expand tab"}
                >
                  <span className="w-3 text-zinc-500 select-none normal-case">
                    {tabOpen ? "▾" : "▸"}
                  </span>
                  <img
                    src={`/talent-icons/UISkillIcon${tg.tabIconId}.png`}
                    alt=""
                    aria-hidden
