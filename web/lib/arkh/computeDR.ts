// ===== ARKH DROP RATE ENTRY POINT =====
// Takes a raw "Copy for Support" envelope, loads it into the Arkh state
// singleton, then runs buildTree(dropRateDesc) to produce the Arkh-style
...
export function computeArkhDropRate(
  rawEnvelope: any,
  charIdx: number,
  mapIdx: number = 0,
  opts?: ComputeDROpts
  ...
  // mapBonData is a module-level export in save/data.ts (matches Arkh's
 ...
 */
export function computeArkhDRPools(
  rawEnvelope: any,
  charIdx: number,
  mapIdx: number = 0,
  opts?: ComputeDROpts
 ...
 */
export function combineDRPools(pools: Record<string, Pool>): ArkhDRResult {
