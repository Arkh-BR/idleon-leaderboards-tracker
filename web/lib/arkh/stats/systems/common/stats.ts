...
//
// Arkh's stats.js is 1380+ lines computing the full TotalStat chain
...
function computeStarSignStatBonuses(
  ...
  let flatTotal = 0,
    pctTotal = 0;
  const flatChildren: ArkhNode[] = [],
    pctChildren: ArkhNode[] = [];
...
function computeAlchBubble(
  ...
  charIdx: number | null,
  saveData: SaveData
): { val: number; children: ArkhNode[]; name?: string } {
...
function computeAllStatPCT(
  charIdx: number,
  ctx: StatCtx
): { val: number; rawSum: number; children: ArkhNode[]; subMissing: string[] } {
  const saveData = ctx.saveData;
  let sum = 0;
  const children: ArkhNode[] = [];
  const subMissing: string[] = [];

  function addSub(name: string, val: number, ch?: ArkhNode[] | null) {
    sum += val;
...
export function computeTotalStat(
  ...
  const flatBaseChildren: ArkhNode[] = [
  ...
  let a4Val = 0;
  let a4bubble: { val: number; children: ArkhNode[]; name?: string } | undefined;
  if (cfg.a4Bubble) {
  ...
  const flatAddChildren: ArkhNode[] = [];
...
// ==================== STATUE BONUS ====================
function _rval(
  resolver: { resolve: (id: any, ctx: any, args?: any) => ArkhNode },
  id: number,
  ctx: any,
  args?: any
): number {
