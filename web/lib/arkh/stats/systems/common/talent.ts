...
// formula via formulaEval. The full Arkh implementation also folds in
...
function buildSummWB19Kids(
  ...
  const battleKids: ArkhNode[] = [];
  for (const group of rawBreakdown.groups) {
    let groupSum = 0;
    const groupKids: ArkhNode[] = group.battles.map((b) => {
...
function computeMaxBookLvParts(saveData: SaveData): {
  ...
  const summKids: ArkhNode[] = buildSummWB19Kids(wb19Parts, saveData);
  const kids: ArkhNode[] = [
 ...
 *  Arkh source resolveAllTalentLVz. */
function resolveAllTalentLVz(
  ...
  let spelunkBonus = 0;
  let superChildren: ArkhNode[] | undefined;
...
export const talent = {
  resolve(id: number, ctx: Ctx, args?: TalentResolveArgs): ArkhNode {
...
// the source dependency map identical to Arkh's, so Stage 3+ can
