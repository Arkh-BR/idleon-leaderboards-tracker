...
//
// grid172Multi mirrors the Arkh source: `1 + gbWith(gl, so, 172, ctx) / 100`,
...
export const equipment = {
  resolve(id: number | string | (number | string)[], ctx: Ctx): ArkhNode {
    ...
    for (let i = 0; i < allSlots.length; i++) {
      ...
      let slotChildren: ArkhNode[] | null = null;
    ...
    if (catalogByType.size > 0) {
      ...
      const catalogChildren: ArkhNode[] = [];
