...
function isEtcBonusWrapper(n: ArkhNode): boolean {
  return /\(etcBonus\s+\d+(?:,\d+)*\)/.test(n.name);
}
function explodeEtcBonus(items: ArkhNode[]): ArkhNode[] {
  const out: ArkhNode[] = [];
  for (const it of items) {
