...
export function loadSaveData(raw: RawEnvelope): void {
  ...
  for (let ci = 0; ci < nChars; ci++) {
    ...
    // PVStatList_N = [STR, AGI, WIS, LUK, level] — arkh reads this for lukCurve
  ...
  // Stash on the singleton so arkh's lukScaling can read raw LUK by char idx.
  ...
  // Derived research/cache state (subset of Arkh — drop-rate doesn't read most)
  ...
  // Tome score — Arkh re-derives this from save data via computeTomeScore
