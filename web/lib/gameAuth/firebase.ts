...
async function authPost(url: string, headers: HeadersInit, body: BodyInit): Promise<any> {
  const r = await fetch(url, { method: "POST", headers, body, signal: AbortSignal.timeout(TIMEOUT_MS) });
...
const bearer = (idToken: string) => ({ headers: { Authorization: `Bearer ${idToken}` }, signal: AbortSignal.timeout(TIMEOUT_MS) });
...
export async function rtdbGet(path: string, idToken: string): Promise<unknown> {
  const r = await fetch(`${RTDB}/${path}.json?auth=${encodeURIComponent(idToken)}`, { signal: AbortSignal.timeout(TIMEOUT_MS) });
  if (!r.ok) throw new Error(`Couldn't read your save (HTTP ${r.status})`);
  return r.json();
}
